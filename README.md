...
Для каждого созданного абонента в системе автоматически создается учетная запись в SSO-сервисе:

- **Логин:** MSISDN абонента (номер телефона) (Например 79000000001)
- **Роль:** ROLE_SUBSCRIBER

При создании абонента через BRT-сервис происходит следующий процесс:
1. BRT-сервис создаёт запись абонента в своей базе данных
2. BRT-сервис отправляет сообщение о создании абонента через RabbitMQ в очередь `subscriber.created`
3. SSO-сервис получает сообщение и автоматически создаёт учетную запись для данного абонента
...
## P.S.
Мой аналитик отправился делать свой проект, поэтому да, увы, без красивых схемок и сваггера. :(

## P.P.S.
Я закоментировал порты в докере, для того, чтобы сымитировать условное нахождение сервисов в закрытой сети,где точкой входа является гейтвей.
